...
public class AutomatonVisualizer extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        // Exemple de transitions pour tester
        transitions.put("q0", Map.of("a", List.of("q1", "q2"), "b", List.of("q3")));
        transitions.put("q1", Map.of("a", List.of("q2")));
        transitions.put("q2", Map.of("b", List.of("q0")));
        transitions.put("q3", Map.of("a", List.of("q1", "q2"), "b", List.of("q3")));

        drawTransitions(pane, transitions);

        Scene scene = new Scene(pane, WIDTH, HEIGHT);
        primaryStage.setTitle("Automaton Visualiser");
        primaryStage.setScene(scene);
        primaryStage.show();
    }

    private void drawTransitions(Pane pane, Map<String, Map<String, List<String>>> transitions) {
        ...
        for (Map.Entry<String, Map<String, List<String>>> entry : transitions.entrySet()) {
            ...
            Map<String, List<String>> transitionMap = entry.getValue();

            for (Map.Entry<String, List<String>> transition : transitionMap.entrySet()) {
                String input = transition.getKey();
                List<String> toStates = transition.getValue();

                int toIndex = 0;
                for (String toState : toStates) {
                    Circle toCircle = stateNodes.get(toState);
                    ...
                    if (fromCircle == toCircle) {
                        ...
                        drawTransition(pane, fromCircle, toCircle, input, toIndex);
                    }
